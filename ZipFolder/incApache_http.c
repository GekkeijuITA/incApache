				   ...
				   char *filename, struct stat *stat_p)
{
	...
	if (fd >= 0)
	{
		...
		/*** TO BE DONE 7.0 START ***/
		ssize_t byte_sent = sendfile(client_fd, fd, NULL, file_size);
		if (byte_sent == -1 || byte_sent != file_size)
